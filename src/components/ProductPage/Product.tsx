    ...
    <>
      <div className={styles.filter__block}>
        ...
        <div className={styles.buttons}>
          ...
          <CustomButton onClick={handleAddCart} title={'Add to Cart'} className='transition-all duration-200 hover:opacity-80 text-center px-3 py-3 text-white font-semibold rounded-[50px] bg-main_blue w-[131px]'/>
      ...
      <Carousel autoplay autoplaySpeed={6000} className={styles.intel__carousel}>
        {lst_bg.map((el, i) => {
          return <CarouselBlock text={el.text} bg={el.image} remark={el.remark} heading={el.heading} />
        })}
      </Carousel>
        ...
        <div className='flex flex-col items-center gap-y-16'>
          ...
          <div className='flex gap-x-[47px] w-[65%]'>
            {lst_cards.map((card, i) => {
              return <CardsProduct image={card.image} text={card.text} />
            })}
          </div>
        </div>
      </div>
